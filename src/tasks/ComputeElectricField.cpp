...
TaskStatus ComputeBaseElectricField(View3 E_base, FieldData_t data_V, const Real En, const Real eta_norm,
  ...
  auto B_d = Kokkos::subview(field_d, Kokkos::ALL, Kokkos::ALL,
      Kokkos::make_pair(component0 + FieldComponents::B, FieldComponents::B+3));
  auto J_d = Kokkos::subview(field_d, Kokkos::ALL, Kokkos::ALL,
      Kokkos::make_pair(component0 + FieldComponents::J, FieldComponents::J+3));
  auto V_d = Kokkos::subview(field_d, Kokkos::ALL, Kokkos::ALL,
      Kokkos::make_pair(component0 + FieldComponents::E, FieldComponents::E+3));
...
TaskStatus ComputeAdjustedElectricField(FieldData_t data, View3 E_base, View3 Jre, const Real En) {
  ...
  Kokkos::parallel_for(
    ...
    Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {NR, NZ}),
    KOKKOS_LAMBDA(const int i, const int j) {
      field_d(i,j,FieldComponent::E + 0) = E_base(i,j,0) - En * Jre(i,j,0);
      field_d(i,j,FieldComponent::E + 1) = E_base(i,j,1) - En * Jre(i,j,1);
      field_d(i,j,FieldComponent::E + 2) = E_base(i,j,2) - En * Jre(i,j,2);
    });
  data.data.device_modify();
